...
class ClickEvent:
    ...
    schema = parse_schema(
        {
            "type": "record",
            "name": "purchase",
            "namespace": "com.udacity.purchase_app",
            "fields": [
    ...
    def serialize(self):
        ...
        #
        #
        # step 1: create a BytesIO object, fastavro has to write into a file-like object
        # here we don't want to save it to disk, write to BytesIO object will keep it in memory
        out = BytesIO()

        # step 2: serialize data with pre-defined schema, then write to memory
        writer(out, ClickEvent.schema, [asdict(self)])

        # step 3: Return bytes containing the entire contents of the buffer.
        # getvalue() method is from io.BytesIO class
        return out.getvalue()
