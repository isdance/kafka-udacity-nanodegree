...
async def consume():
    ...
    # Define the appropriate headers, in this case we want return data in JSON format
    #       See: https://docs.confluent.io/current/kafka-rest/api.html#content-types
    headers = {"Content-Type": "application/vnd.kafka.json.v2+json"}
    # Define the consumer group creation payload
    # Note: "format": "avro" is used for step 3, which is in the Accept header of subsequent read API requests performed against this consumer.
    ...
    # Step 01: POST to the appropiate endpoint to create a consumer group
    #       See: https://docs.confluent.io/current/kafka-rest/api.html#post--consumers-(string-group_name)
    resp = requests.post(
    ...
    # Here the response contains the base_uri and instance_id we need for next steps
    resp_data = resp.json()
    ...
    #
    # Step 02: POST the subscription payload to active a subscription
    #       See: https://docs.confluent.io/current/kafka-rest/api.html#consumers
    #
    resp = requests.post(
    ...
    while True:
        ...
        #
        # Step 3: Begin fetching records in Avro format
        #       See: https://docs.confluent.io/current/kafka-rest/api.html#get--consumers-(string-group_name)-instances-(string-instance)-records
        #
